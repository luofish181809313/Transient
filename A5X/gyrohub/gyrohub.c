...
static int gyro_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	printk("yuduan.xie--gyro_recv_data\n");
...
static int gyrohub_probe(struct platform_device *pdev)
{
	...
	printk("yuduan.xie--gyrohub_probe--begin\n");
	pr_debug("%s\n", __func__);
	...
	*/
	...
	gyrohub_init_flag = 0;
	printk("yuduan.xie--gyrohub_probe--end\n");
	pr_debug("%s: OK\n", __func__);
	return 0;
...
{
	gyroPltFmDev = pdev;
	printk("yuduan.xie--gyrohub_local_init--begin\n");
	...
	if (-1 == gyrohub_init_flag)
		return -1;
	printk("yuduan.xie--gyrohub_local_init--end\n");
	return 0;
}
static struct gyro_init_info gyrohub_init_info = {
	.name = "gyrohub",
